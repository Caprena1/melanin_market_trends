...
#COMBINED NEW DATASET BY USING CONCAT AND MERGE(JOIN)
melanin_newdata = pd.merge(columns_from_ec_df, columns_from_bp_df, left_on='Brand', right_on='Brand')
melanin_newdata = pd.concat([columns_from_ec_df, columns_from_bp_df], axis=1)
...
print(columns_from_ec_df.columns)
print(columns_from_bp_df.columns)
